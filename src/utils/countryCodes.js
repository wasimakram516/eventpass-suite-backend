const COUNTRY_CODES = [
    ...
    { code: "+225", country: "Côte d'Ivoire", isoCode: "ci", digits: 8 },
...
const DEFAULT_ISO_CODE = "om";

const getCountryByCode = (code) => {
    return COUNTRY_CODES.find((cc) => cc.code === code) || null;
};
...
const extractCountryCodeAndIsoCode = (phone) => {
    if (!phone || typeof phone !== "string") {
        return { countryCode: null, isoCode: null, localNumber: phone || null };
    }

    const phoneStr = phone.trim();
    if (!phoneStr.startsWith("+")) {
        return { countryCode: null, isoCode: null, localNumber: phoneStr };
    }
    ...
    if (foundCountry) {
        const localNumber = phoneStr.substring(foundCountry.code.length).trim();
        return {
...
const combinePhoneWithCountryCode = (localNumber, isoCode) => {
    if (!localNumber) return null;
    if (!isoCode) return localNumber;
