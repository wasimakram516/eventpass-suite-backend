...
exports.getEventBySlug = asyncHandler(async (req, res) => {
    ...
    const event = await Event.findOne({ slug, eventType: ALLOWED_EVENT_TYPE })
        ...
        .populate("linkedEventRegId", "formFields startDate endDate venue");
...
exports.restoreAllEvents = asyncHandler(async (req, res) => {
    ...
    return response(
        res,
        200,
        `Restored ${restoredCount} DigiPass events${skippedCount ? `, skipped ${skippedCount} due to slug conflict` : ""}`
    );
});
