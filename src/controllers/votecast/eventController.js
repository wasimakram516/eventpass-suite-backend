...
const ALLOWED_EVENT_TYPE = "votecast";
...
exports.getEventDetails = asyncHandler(async (req, res) => {
    ...
    return response(res, 200, "VoteCast Events fetched successfully", {
        events: eventsWithPollCount,
        totalEvents: eventsWithPollCount.length,
    });
});
    ...
    if (background) {
        try {
            ...
            if (Object.keys(parsedBackground).length > 0) {
                if (parsedBackground.en !== undefined) {
                    event.background = event.background || {};
                    event.background.en = parsedBackground.en || event.background.en || {};
                }
                if (parsedBackground.ar !== undefined) {
                    event.background = event.background || {};
                    event.background.ar = parsedBackground.ar || event.background.ar || {};
                }
            }
        } catch (err) {
...
exports.deleteEvent = asyncHandler(async (req, res) => {
    ...
    return response(res, 200, "VoteCast event moved to recycle bin");
});
...
exports.restoreEvent = asyncHandler(async (req, res) => {
    ...
    return response(res, 200, "VoteCast event restored successfully", event);
});
...
exports.permanentDeleteEvent = asyncHandler(async (req, res) => {
        ...
        for (const option of poll.options || []) {
            if (option.imageUrl) {
                try {
                    ...
                    console.error("Failed to delete poll option image from S3:", err);
                }
            }
        }
    }
    await Poll.deleteMany({ eventId: event._id });
    ...
    return response(
        res,
        200,
        "VoteCast event and its polls permanently deleted"
    );
});

// Permanent delete ALL votecast events (cascade delete polls)
exports.permanentDeleteAllEvents = asyncHandler(async (req, res) => {
    const events = await Event.findDeleted({ eventType: ALLOWED_EVENT_TYPE });
    if (!events.length) {
        return response(res, 404, "No VoteCast events found in trash to delete");
    }

    const eventIds = events.map((ev) => ev._id);

    const polls = await Poll.find({ eventId: { $in: eventIds } });
    for (const poll of polls) {
        for (const option of poll.options || []) {
            if (option.imageUrl) {
                try {
    ...
    await Poll.deleteMany({ eventId: { $in: eventIds } });
    ...
    return response(
        res,
        200,
        `Permanently deleted ${result.deletedCount} VoteCast events and their polls`
    );
});
